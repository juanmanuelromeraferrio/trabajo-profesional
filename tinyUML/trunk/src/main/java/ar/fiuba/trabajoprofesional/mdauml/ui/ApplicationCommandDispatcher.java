...
public class ApplicationCommandDispatcher implements AppCommandListener {
  ...
  private void initSelectorMap() {
    try {
      ...
      selectorMap.put("HELP_CONTENTS", new MethodCall(getClass().getMethod("displayHelpContents")));
  ...
  private boolean canCreateNewModel() {
    if (appState.isModified()) {
      return JOptionPane.showConfirmDialog(getShellComponent(), ApplicationResources.getInstance()
          .getString("confirm.new.message"),
  ...
  private boolean canOpen() {
    if (appState.isModified()) {
      return JOptionPane.showConfirmDialog(getShellComponent(), ApplicationResources.getInstance()
          .getString("confirm.open.message"),
  ...
  public void delete() {
    if (appState.getCurrentFocusedComponent() instanceof DiagramTree) {
      ...
      if (tree.getSelectionPath() != null) {
        ...
        DeleteDiagramCommand command =
            new DeleteDiagramCommand(appState.getUmlModel(), (UmlDiagram) node.getUserObject());
  ...
  public void displayHelpContents() {
    try {
      ...
      JOptionPane.showMessageDialog(getShellComponent(), getResourceString("error.nohelp.message"),
